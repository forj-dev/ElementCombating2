...
public class ElementDamageCrystalEntity extends Entity implements IAnimatable {
    private static final TrackedData<Optional<ElementType>> ELEMENT_TYPE = DataTracker.registerData(ElementDamageCrystalEntity.class, Utils.OptionalElementTypeTrackedDataHandler);
    private int lastTicks = -1;
    private int ownerId = -1;

    private final AnimationFactory factory = new AnimationFactory(this);

    public ElementDamageCrystalEntity(EntityType<?> type, World world) {
        super(type, world);
    }

    public ElementDamageCrystalEntity(World world, LivingEntity owner, int lastTicks) {
        super(Entities.ELEMENT_DAMAGE_CRYSTAL_ENTITY, world);
    ...
    public void tick() {
        ...
        if (!this.hasNoGravity()) {
            this.setVelocity(this.getVelocity().multiply(0.9).add(0.0, -0.03, 0.0));
        }
        this.move(MovementType.SELF, this.getVelocity());
        if (this.world.isClient) return;
        if (getElementType() == null) {
            setElementType(Elements.Sculk);
        }
        ...
        for (LivingEntity entity : entities) {
            if (entity.equals(getOwner())) {
                this.discard();
                ((StatAccessor) entity).getFlagManager().add(new ElementDamageFlag(80, getElementType()));
                return;
            }
        }
    }
    ...
    public static class ElementDamageFlag implements Flag {
        private int lastTicks;
        public final ElementType type;

        public ElementDamageFlag(int lastTicks, ElementType type) {
            this.lastTicks = lastTicks;
            this.type = type;
        }
